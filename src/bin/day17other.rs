...
impl Simulation {
    ...
    fn display(&self) {
        for y in (0..=self.height+4).rev().take(20) {
            print!("|");
            for x in 0..=6 {
                print!(
                    "{}",
                    if self.solid.get(&point!(x, y)).is_some() {
                        '#'
                    } else {
                        '.'
                    }
        ...
        loop {
            ...
            if self.free(rock, point!(0, -1)) {
                if let Some(prev) = self.cache.insert(
                    ...
                    dbg!(rock, jet, self.dropped, self.height, cycle, height);
...
fn solve(input: &str) -> i64 {
    ...
    loop {
        if let Some((cycle, height)) = simulation.drop() {
            ...
            let skipped_height= cycles * height;  
            dbg!(rocks, cycles, cycle, height, skipped_height);

            simulation.display();
            // Drop the rocks that are not part of a cycle.
            for _ in 0..rocks % cycle {
                let jet = simulation.turn % simulation.jets.len();
                let rock = Rock::index(simulation.dropped);
                println!("{}: {:?} {}", simulation.dropped, rock, jet);
                simulation.drop();
            }
